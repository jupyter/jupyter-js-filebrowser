...
var FileBrowserViewModel = (function () {
    ...
    FileBrowserViewModel.prototype.rename = function (path, newPath, overwrite) {
        ...
        return this._contents.get(newPath, {}).then(function (content) {
            throw new Error("\"" + newPath + "\" already exists");
        }, function () {
            return _this._rename(path, newPath);
        });
    };
    /**
     ...
     */
    FileBrowserViewModel.prototype._rename = function (path, newPath) {
        var _this = this;
     ...
     */
    FileBrowserViewModel.prototype._upload = function (file) {
        var _this = this;
...
var FileBrowser = (function (_super) {
    ...
    function FileBrowser(model) {
        ...
        this._buttons[Button.Refresh].onclick = function () { _this._model.refresh(); };
            ...
            this._model.rename(name, path + name).catch(function (error) {
                if (error.message.indexOf('already exists') !== -1) {
                    ...
                    phosphor_dialog_1.showDialog(options).then(function (button) {
                        if (button.text === 'OK') {
                            _this._model.rename(name, path + name, true);
                        }
                    });
                }
                else {
     ...
     */
    FileBrowser.prototype._getPath = function (event) {
        var index = hitTestNodes(this._items, event.clientX, event.clientY);
        var path = '';
        if (index !== -1) {
    ...
    FileBrowser.prototype._doRename = function (row) {
        ...
        doRename(row, text, this._editNode).then(function (changed) {
            ...
            _this._model.rename('./' + content, './' + text.textContent).catch(function (error) {
                if (error.message.indexOf('already exists') !== -1) {
                    var options = {
                        title: 'Overwrite file?',
                        host: _this.node,
