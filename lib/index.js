...
var FileBrowser = (function (_super) {
    ...
    FileBrowser.prototype.onUpdateRequest = function (msg) {
        ...
        updateCrumbs(this.node.firstChild, this._crumbs, this._crumbSeps, this._model.path);
    };
    /**
    ...
    FileBrowser.prototype._handleCrumbEvent = function (event) {
        // Stop the event propagation.
        event.preventDefault();
        event.stopPropagation();
        // Find the matching node.
        var i = hitTestNodes(this._crumbs, event.clientX, event.clientY);
        if (i == Crumb.Home) {
            this._model.path = '';
            return;
        }
 ...
 */
function updateCrumbs(node, breadcrumbs, separators, path) {
